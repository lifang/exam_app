...
Validator.messageSource['zh-cn'] = [
    ...
    ['validate-alphanum-chinese' , '请输入中文、英文字母、数字或者下划线,其它字符是不允许的.'],
...
Validator.prototype = {
    ...
    error : function(v,elm,useTitle) {
        ...
        if(typeof error == 'string') {
            if(v) args.push(v.replace(/[^\x00-\xff]/g, "rr" ).length);
...
Validation.addAllThese([
    ...
    ['validate-alphanum-chinese',/^[a-zA-Z0-9\u4e00-\u9fa5_]+$/],
...
Validation.addAllThese([
    ...
    ['min-length',function(v,elm,args,metadata) {
        return v.replace(/[^\x00-\xff]/g, "rr" ).length >= parseInt(args[0]);
    }],
    /*
    ...
    ['validate-phone',/^(0[1-9]{2}[0-9])?(0[12][0-9])?\d{6,8}$/],
