...
var nicEditorInstance = bkClass.extend({
    ...
    init : function() {
        ...
        if(this.getContent() == "") {
            this.setContent('');  //原来的内容为：this.setContent('<br />');
        }
...
var nicUploadButton = nicEditorAdvancedButton.extend({	
    ...
    addForm : function() {
        ...
        myDoc.write('<form method="post" action="'+this.uri+'/upload_images?id='+this.myID+'" enctype="multipart/form-data">');
        myDoc.write('<input type="hidden" name="APC_UPLOAD_PROGRESS" value="'+this.myID+'" />');
        myDoc.write('<div style="font-size: 14px; font-weight: bold; padding-top: 5px;">请选择图片</div>');
    ...
    startUpload : function() {
        ...
        this.myStatus.setContent('<img src="/images/ajax-loader.gif" style="float: right; margin-right: 40px;" /><strong>上传中...</strong><br />请等待');
        this.myForm.submit();
        //this.removePane();
        //nicUploadButton.statusCb(true);
    ...
    makeRequest : function() {
        if(this.pane && this.pane.pane) {
            ...
            nicUploadButton.statusCb({
                'url':this.myFrame.contentWindow.document.getElementsByTagName("img")[0].src
            });
...
var nicFlashButton = nicEditorAdvancedButton.extend({
    addPane : function() {
        this.flash = this.ne.selectedInstance.selElm().parentTag('embed');
        this.addForm({
            '' : {
                type : 'title',
                txt : '添加/修改 falsh'
    ...
    submit : function(e) {
        var src = this.inputs['src'].value;
        this.form = this.pane.pane.getElementsByTagName("form")[0];
        if(src == "" || src == "http://") {
            alert("您必须输入一个有效的url地址。");
            return false;
        }
        this.removePane();
        if(!this.flash) {
        ...
        if(this.flash) {
            this.flash.innerHTML = "<embed src='"+ this.inputs['src'].value +"' wmode='opaque'></embed>";
        }
    }
});
...
/* END CONFIG */
var nicMediaButton = nicEditorAdvancedButton.extend({
    addPane : function() {
        this.media = this.ne.selectedInstance.selElm().parentTag('IMG');
        this.addForm({
            '' : {
                type : 'title',
                txt : '添加/修改 音/视频'
    ...
    submit : function(e) {
        ...
        if(!this.media) {
            if(src.substring(src.length-3,src.length)=="swf") {
                ...
                var div = document.createElement("div");  //新div
                this.form.appendChild(div);
                div.innerHTML = "<img title='播放' alt='~http://back_server_path~"+src+"' src='/images/paper/play_icon.png' id='audio_control_x' onclick=\"javascript:audio_play('x','~http://back_server_path~"+src+"');\"/>";
                this.ne.nicCommand("insertHtml",div.innerHTML);
            }
        }
        if(this.media) {
