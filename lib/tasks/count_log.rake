...
namespace :count_log do
  ...
  task(:excel => :environment) do
    ...
    str ="select count(ex.id) sum,ex.types from exam_users eu 
          left join examinations ex on ex.id=eu.examination_id
          where eu.created_at>=? and eu.created_at <=? group by ex.types"
    ...
    all_info=ExamUser.find_by_sql("select count(ex.id) sum,ex.types from exam_users eu
      left join examinations ex on ex.id=eu.examination_id group by ex.types")
    ...
    if(!File.exist?(url))
      ...
      sheet.row(1).concat ["#{1.day.ago.strftime('%Y%m%d')}", "#{new_user}/#{User.all.count}", 
        "#{new_vip}/#{Order.all.count}","#{combine_practice}/#{all_combine}","#{true_practice}/#{all_true}",
        "#{simulate_exam}/#{all_simulate}"]
      ...
      puts row_num
      new_sheet.row(row_num).concat ["#{1.day.ago.strftime('%Y%m%d')}", "#{new_user}/#{User.all.count}", 
        "#{new_vip}/#{Order.all.count}","#{combine_practice}/#{all_combine}",
        "#{true_practice}/#{all_true}","#{simulate_exam}/#{all_simulate}"]
