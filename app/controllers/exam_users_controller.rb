class ExamUsersController < ApplicationController
  def create_exam_user
    ...
    if (User.find_by_name(params[:infoname])!=nil || User.find_by_email(params[:infoemail])!=nil)
      @user=User.find_by_name(params[:infoname]).id
      if (ExamUser.find_by_user_id(@user).examination_id ==Examination.find(params[:examination_id].to_i).id )
        flash[:email_err]="该考生信息已加入"
      else
        exam_user = ExamUser.create!(:user_id=>@user,:examination_id=>params[:examination_id],:password=>"123456",
