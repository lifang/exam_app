...
class ProblemsController < ApplicationController
  ...
  def create
    ...
    @problem.update_attributes(:title=>@problem.title.gsub("audio_play('x')","audio_play(#{@problem.id})").gsub("id=\"audio_x\"","id='audio_#{@problem.id}'").gsub("id=\"audio_control_x\"","id='audio_control_#{@problem.id}'").gsub("problem_x_dropplace","problem_#{@problem.id}_dropplace").gsub("problem_x_writeplace","problem_#{@problem.id}_writeplace"))
    #创建题点
    score_arr = {}
