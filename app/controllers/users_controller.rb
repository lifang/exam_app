class UsersController < ApplicationController
  ...
  def show
    @user = User.find(params[:id])
    @examinations = Examination.find_by_sql("select eu.total_score,p.total_question_num,
ex.*,eu.answer_sheet_url from exam_users eu inner join examinations ex on ex.id=eu.examination_id
inner join papers p on p.id=eu.paper_id where user_id=#{@user.id} order by start_at_time desc").paginate(:page=>params[:page],:per_page=>10)
  end
