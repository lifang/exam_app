class UsersController < ApplicationController
  ...
  def index_search
    if params[:user_name]==""&&params[:user_email]==""
      ...
      if params[:user_name]!=""&&params[:user_email]==""
        @users = User.find_by_sql("select * from users where name='#{params[:user_name]}'")
      else
        if params[:user_name]==""&&params[:user_email]!=""
          @users = User.find_by_sql("select * from users where email='#{params[:user_email]}'")
        else
          @users = User.find_by_sql("select * from users where email='#{params[:user_email]}' and name='#{params[:user_name]}'")
        end     
      end
    end
    render "index"
      ...
      def set_role
        ...
        @user_role_relations = User.find(@user_id).user_role_relations
        (1..@roles_num).each do |id|
          if params["check_box#{id}"]!=nil && params["check_box#{id}"] != ""
            if @user_role_relations.find_by_role_id(params["check_box#{id}"].to_i)==nil
              UserRoleRelation.create(:user_id=>@user_id,:role_id=>params["check_box#{id}"].to_i)
            end
          else
            if @user_role_relations.find_by_role_id(id)!=nil
              @user_role_relations.find_by_role_id(id).destroy
            end
          end
